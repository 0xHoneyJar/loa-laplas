...
if [[ -n "$MODULE_PATH" ]]; then
    ...
    if ( cd "$SUBSTRATE_ROOT" && node "$LAPLAS_READY" "$_abs_module" ) >/dev/null 2>"$_ready_err"; then
        ...
        if [[ -f "$_seeder" ]]; then
            ...
            # T3 session->run link (the last mile): bind THIS CC session to the
            # armed run so both prompt-arm (gradient injection) and the exit-gate
            # (Stop) resolve run_id from its session_id. 9x7 (arm-on-entry): prompt-
            # arm no longer ADOPTS the most-recent run or writes this link — it only
            # READS it. The DISPATCHER is the sole writer (gate 0), the one actor
            # holding BOTH the session id (CLAUDE_CODE_SESSION_ID == the Stop
            # payload's session_id) AND the freshly-minted run_id. {run_id,armed_at}
            # is the shape prompt-arm + the exit-gate read by-session/<session> verbatim.
