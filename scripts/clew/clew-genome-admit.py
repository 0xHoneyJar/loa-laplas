...
def _rewrite_ledger_atomic(path: Path, lines: list[str], idx: int, new_entry: dict) -> None:
    ...
    # Write the lines verbatim (only idx replaced). Do NOT filter "blank" lines —
    # that silently mutates unrelated ledger content (cross-model iter-2, gemini LOW).
    tmp.write_text("\n".join(out) + "\n", encoding="utf-8")
    tmp.replace(path)
...
def main(argv: list[str]) -> int:
    ...
    # Entry structure FIRST (cross-model iter-3, codex): a valid ledger entry ALWAYS
    # carries target.construct (capture schema requires it). A missing one is a
    # malformed/tampered entry — loud ERROR, not admitted, checked BEFORE the run gates
    # so it can never be misclassified as a mere run-failure suspect (and never admitted).
    clew_construct = (entry.get("target") or {}).get("construct")
    if not clew_construct:
        return emit({"status": "error",
                     "reason": "clew has no target.construct — malformed entry, NOT admitted "
                               "(a valid ledger entry always carries target.construct; fix the ledger)"}, 1)
    ...
    # --- Load the DESTINATION manifest ONCE (needed by both the F1 construct
    # binding and the genome computation). ------------------------------------
    ...
    dest_slug = manifest.get("slug")

    # F1 PARTIAL mitigation — the remaining TWO gates (gate (a), target.construct
    # presence, already fired right after the entry loaded). A borrowed/roleplayed clew
    # must fail ALL:
    #   (b) the clew's construct MUST equal the DESTINATION manifest slug — an A-clew
    #       cannot be distilled into B's genome even with A's run_id (cross-construct
    #       injection). A manifest with NO slug is unbindable → hard ERROR, NEVER a skip
    #       (cross-model iter-3, claude HIGH: `if dest_slug and ...` silently skipped
    #       gate (b) for an unslug'd manifest).
    #   (c) the verified run's envelopes MUST include that construct_slug — defeats
    #       "borrow ANY valid run_id". Envelope-naming contract: compose-handoff-wrap.sh
    #       writes <run>/envelopes/*.handoff.json (the same set compose-verify-run.sh gates).
    # Full content<->run binding (this clew was PRODUCED by that run) = bd-70m.
    if not dest_slug:
        return emit({"status": "error",
                     "reason": f"destination construct.yaml ({yaml_path}) has no slug — cannot "
                               f"bind the clew to a construct; fix the manifest"}, 1)
    if clew_construct != dest_slug:
        return quarantine(
            f"clew target.construct '{clew_construct}' != destination slug '{dest_slug}' "
            f"(cross-construct injection)", verdict)
    ...
    if env_dir and env_dir.is_dir():
        for ef in sorted(env_dir.glob("*.handoff.json")):
            try:
                ...
                pass
    if clew_construct not in run_constructs:
        return quarantine(
    ...
    # WRITE ORDER (cross-model iter-2, codex HIGH / claude MED): the ledger entry is
    # written + validated BEFORE construct.yaml. If the process dies between the two,
    # the ledger is AHEAD of the manifest → genome-integrity verify catches it
    # (recomputed head != manifest head) and FAILS LOUDLY. The reverse order would
    # advance the manifest head with no ledger link, and a re-run would read the
    # advanced depth and mint a PHANTOM link — silent corruption. genome_seq=new_depth
    # is the authoritative chain index (verify orders by it, not the mutable distilled_at).
