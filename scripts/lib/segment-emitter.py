...
const dagValidate = (items) => {
  ...
  for (const it of items) {
    ...
    if (it.id === "__proto__" || it.id === "constructor" || it.id === "prototype") return "item id '" + it.id + "' is a reserved object key — rejected (mirrors the output_schema prototype-magic guard; '__proto__' specifically would be silently lost as a result key)";
